"""
Asana Context Agent — FastAPI A2A server
Exposes POST /a2a and GET /.well-known/agent.json
Refreshes Asana MCP OAuth token before each request.
"""
...
logger = logging.getLogger("asana-context")

app = FastAPI(title="Asana Context Agent")
...
async def a2a_endpoint(request: Request):
    ...
    if len(content.encode()) > 10_000:
        return JSONResponse(status_code=400, content={"error": "Payload too large"})

    logger.info(f"[asana-context] Received request id={rpc_id}")
    ...
    logger.info(f"[asana-context] Completed request id={rpc_id}")
    return JSONResponse(
        content={
            "jsonrpc": "2.0",
            "id": rpc_id,
