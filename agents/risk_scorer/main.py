"""
Risk Scorer Agent — FastAPI A2A server
...
app = FastAPI(title="Risk Scorer Agent")
...
async def a2a_endpoint(request: Request):
    ...
    logger.info(f"[risk-scorer] Received request id={rpc_id}")
    ...
    logger.info(f"[risk-scorer] Completed request id={rpc_id}")
    return JSONResponse(
        content={
            "jsonrpc": "2.0",
            "id": rpc_id,
